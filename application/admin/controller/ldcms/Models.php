...
class Models extends Base
{
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            $this->success('', null, ['ismenu' => $params['ismenu']]);
        }
...
假设：需要新增一个新闻模型，那么对应表只需填写news即可。<br>不需要填写前缀，最终会生成<code>fa_ldcms_document_news</code>表。
    ...
    public function edit($ids = null)
    {
        if ($this->request->isPost()) {
            ...
            $oldismenu = $row['ismenu'];
            $newismenu = $params['ismenu'];
            ...
            if (!$params['status']) {
                $newismenu = $params['status'];
            }
            $ismenu = $oldismenu != $newismenu;

            $this->success('ok', null, ['ismenu' => $ismenu]);
        }
...
假设：需要新增一个新闻模型，那么对应表只需填写news即可。<br>不需要填写前缀，最终会生成<code>fa_ldcms_document_news</code>表。
    ...
    public function copy($ids = null)
    {
        ...
        if ($ids) {
            ...
            $this->success('', null, ['ismenu' => $data['ismenu']]);
        }
