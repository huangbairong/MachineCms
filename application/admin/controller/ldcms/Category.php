...
class Category extends Base
{
    ...
    public function add()
    {
        ...
        return Form::instance()
            ->fieldShow()
            ->setGroup('基础')
            ->setFormItem('type', __('Type'), 'radio', 'checked', ['content_list' => [ 0 => '模型',1 => '链接'],'value' => 0])
            ->setFormItem('pid', __('Pid'), 'select', '', ['content_list' => $categorys,'value' => $categoryvalue,'search' => true])
    ...
    public function edit($ids = null)
    {
        ...
        return Form::instance()
            ...
            ->setFormItem('type', __('Type'), 'radio', 'checked', ['content_list' => [ 0 => '模型',1 => '链接']])
            ->setFormItem('pid', __('Pid'), 'select', '', ['content_list' => $categorys,'search' => true])
            ->setFormItem('name', __('Name'), 'string', 'required')
            ->setFormItem('subname', __('Subname'), 'string')
            ->setFormItem('ename', __('Ename'), 'string')
            ->setFormItem('mid', __('Mid'), 'select', 'required', ['content_list' => $this->model_names,'visible' => 'type=0'])
    ...
    public function multi($ids = null)
    {
        ...
        try {
            ...
            if (array_key_exists('is_home', $values)) {
                $row = $this->model->where($this->model->getPk(), 'in', $ids)->find();
                if (!$row['model_table_name']) {
                    $modelsModel = new ModelsModel();
                    $modelTableName = $modelsModel->where('id', $row['mid'])->value('table_name');
                    $row['model_table_name'] = $modelTableName;
                    $row->save();
                }
                $this->model->where('lang', $this->lang)->where('mid', $row['mid'])->update(['is_home' => 0]);
            }
            foreach ($list as $item) {
        ...
        if ($this->request->isPost()) {
            ...
            try {
                ...
                foreach ($fieldtexts as $fieldtext) {
                    if (empty($row[$fieldtext])) {
                        continue;
                    }

                    $textdata[$fieldtext] = trim($row[$fieldtext]);
                }
                /*批量翻译字段内容*/
                ...
                foreach ($fieldhtmls as $fieldhtml) {
                    ...
                    usleep(1500000);
                    $up_data[$fieldhtml] = $youdaoapi->translateHtml($row[$fieldhtml], $from, $to);
                }
                $row->save($up_data);
