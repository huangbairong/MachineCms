...
trait LdRoute
{
        ...
        if ($this->getName() == 'ldcms') {
            ...
            if ($mode == 1 && !empty($urlData['path']) && $isDirRoute) {
                ...
                Route::rules([
                    ...
                    '*'       => [],
                    // 其它保持原样
                    'alias'   => Route::rules(true)['alias'],
                    'domain'  => Route::rules(true)['domain'],
                    'pattern' => Route::rules(true)['pattern'],
                    'name'    => Route::rules(true)['name'],
                ]);
                Route::rule($rules);
    ...
    protected function customCategoryRoute($config)
    {
        $customUrlname = $config['custom_urlname'];
        if (empty($customUrlname)) {
            return false;
        }
        $rules = [];
        $execute = "\\think\\addons\\Route@execute?addon=%s&controller=%s&action=%s";
        foreach ($customUrlname as $k => $v) {
            $urlArr = explode('/', $v);
            if (count($urlArr) < 3) {
                continue;
            }
