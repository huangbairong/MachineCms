...
class Base extends Frontend
{
    ...
    protected function getUrlName()
    {
        ...
        // 获取自定义的栏目url
        $custom_urlname = $this->addonConfig['custom_urlname'];
        $custom_urlname = array_keys($custom_urlname);
        $path = $this->request->path();
        // 去掉 path 中的 '/' 和 id 部分
        $path = trim($path, '/'); // 去除首尾斜杠
        if ($id) {
            // 如果 path 以 id 结尾，去掉 id
            if (substr($path, -strlen($id)) === (string)$id) {
                $path = substr($path, 0, -strlen($id));
                $path = rtrim($path, '/'); // 去掉末尾斜杠
            }
        }
        if (in_array($path, $custom_urlname)) {
            return $path;
        }
        //没有则获取变量
        return $this->request->param('category');
    }
}
