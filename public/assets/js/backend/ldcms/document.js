define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function ($, undefined, Backend, Table, Form) {
    ...
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    ...
                    translate_url: 'ldcms/document/translate/mid/' + Config.mid,
            ...
            var columns = [
                ...
                { field: 'cname', title: __('Cid'), width: 120, operate: false },
                {
                    field: 'title', title: __('Title'), operate: 'LIKE', align: 'left', customField: 'flag', formatter: function (value, row, index) {
                        value && value.length > 20 ? tit = value.substring(0, 20) + '...' : tit = value; //截取20个字符
        ...
        translate: function () {
            function batch_translate(id, from_lang, to_lang, callback) {
                var url = 'ldcms/document/translate/mid/' + Config.mid + "?id=" + id;
                $.ajax({
                    url: url,
                    type: 'POST',
                    dataType: 'json',
            ...
            Form.api.bindevent($("form[role=form]"), '', '', function () {
                ...
                var from_lang = $('[name=from_lang]').val();
                var to_lang = $('[name=to_lang]').val();
                if (!ids) {
                    Toastr.error(__('请先选择数据'));
                    return false;
                }
                ids = ids.split(',');
                Layer.load(0)                // 逐个调用batch_translate进行翻译
                function translateNext(index) {
                    ...
                    batch_translate(ids[index], from_lang, to_lang, function () {
                        if (window.parent && window.parent.$) {
                            window.parent.$("#table").bootstrapTable('refresh');
                        }
                        setTimeout(() => {
